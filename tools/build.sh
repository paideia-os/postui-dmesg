#!/usr/bin/env bash
# postui-dmesg -- per-repo build script.
#
...
# elf64`. At M1-001 postui-dmesg's `sources:` list holds a single entry
# (src/klog_poll.pdx); the M1-002..M4 landings each grow the set. The
# final link into build-out/postui-dmesg is a DOWNSTREAM packaging
# step that lands with M1-005 (_start binder + Main::main); until then
# this script produces loose objects only. Mirrors
# ../postui-top/tools/build.sh byte-for-byte modulo the src/widgets
# and src/input subdirectory fan-out (postui-dmesg does not carry
# those subdirectories at v1).
#
...
#
# Requires paideia-as >= 0.34.0. postui-dmesg's manifest pins that
# floor: 0.34 is the first release that carries the full mnemonic
# surface postui (this repo's primary dependency) depends on, and
# postui-dmesg's own poll module inherits the same encoder invariants
# (2-op imul reg,reg, single-line pub let string literals,
# module-basename PascalCase enforcement, tightened test-mnemonic
# reservation).
...
usage: tools/build.sh
  ...
  postui-dmesg grows a satellite variant.
EOF
            exit 0
            ;;
        *)
